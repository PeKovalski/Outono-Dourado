...
document.addEventListener('DOMContentLoaded', async function() {
    // Inicializa Supabase
    if (!window.supabase) {
        console.error('Supabase não carregado. Verifique o CDN.');
        showToast('Erro', 'Falha ao carregar o Supabase. Tente novamente mais tarde.', 'error');
        return;
    }
    const supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY);

    // Inicializa elementos do mascote
    const mascotText = document.getElementById('mascot-text');
    const mascotMessageBox = document.getElementById('mascot-message');
    if (!mascotText || !mascotMessageBox) {
        console.error('Elementos do mascote não encontrados no DOM.');
        return;
    }
    ...
    function initializeMascot() {
        ...
        setInterval(() => {
            if (!mascotMessageBox.style.display || mascotMessageBox.style.display === 'none') {
                toggleMascotMessage();
            }
        }, 30000);
    }
    ...
    async function loadProductsFromSupabase() {
        ...
        if (error) {
            ...
            showToast('Erro', 'Não foi possível carregar os produtos. Tente novamente.', 'error');
            return;
        }
    ...
    function renderProducts(products) {
        ...
        if (filteredProducts.length === 0) {
            grid.innerHTML = `<div style="grid-column: 1 / -1; text-align: center; padding: 2rem;">Nenhum produto encontrado</div>`;
            return;
        }
        ...
        grid.innerHTML = filteredProducts.map(product => `
            <div class="product-card">
                ...
                <div class="product-content">
                    ...
                    <button class="btn-secondary view-details" onclick="openProductDetail('${product.id}')">
                        Ver Detalhes
                    </button>
                </div>
            </div>
    ...
    async function login(email, password) {
        ...
        showToast('Login bem-sucedido!', `Bem-vindo, ${data.user.user_metadata.name}!`, 'success');
        closeLoginModal();
    }
    ...
    async function saveOrderToSupabase(order) {
        if (!currentUser) {
            showToast('Erro', 'Você precisa estar logado para finalizar o pedido.', 'error');
            return;
        }
        ...
        showToast('Pedido salvo!', `Número: ${data[0].id}`, 'success');
    }
...
// ... Adicione aqui outras funções do seu script.js original (carrinho, modais, etc.) ...
